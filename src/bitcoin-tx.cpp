...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " lightcoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  lightcoin-tx [options] <hex-tx> [commands]  Update hex-encoded lightcoin transaction\n" +
            "or:     lightcoin-tx [options] -create [commands]   Create hex-encoded lightcoin transaction\n" +
            "\n";
