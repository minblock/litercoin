...
const char* BIP71_MIMETYPE_PAYMENT = "application/lightcoin-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/lightcoin-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/lightcoin-paymentrequest";
#endif
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start lightcoin: click-to-pay handler"));
        }
        else {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("lightcoin://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'lightcoin://' is not a valid URI. Use 'lightcoin:' instead."),
    ...
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Lightcoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
