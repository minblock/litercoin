Lightercoin Core version 0.18.1 is now available from:
...
`/Applications/Lightercoin-Qt` (on Mac) or `litecoind`/`litecoin-qt` (on
Linux).
...
Lightercoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Lightercoin Core on unsupported systems.
...
10.10. Additionally, Lightercoin Core does not yet change appearance when
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@litecoin.org) the Lightercoin Core
...
- A new document introduces Lightercoin Core's BIP174 [Partially-Signed
  Lightercoin Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Lightercoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Lightercoin Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Lightercoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Lightercoin Core, subsequently importing
...
- A new `litecoin-wallet` tool is now distributed alongside Lightercoin
...
This section describes planned changes to Lightercoin Core that may affect
other Lightercoin software and services.

- Since version 0.16.0, Lightercoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Lightercoin Core 0.20 (expected about
  a year after 0.18), Lightercoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Lightercoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Lightercoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Lightercoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Lightercoin Core
...
- A sub-project of Lightercoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Lightercoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Lightercoin Core's own implementation, although entropy
  gathered by Lightercoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Lightercoin Core a
...
- On macOS, Lightercoin Core now opts out of application CPU throttling
