Litercoin Core version 0.18.1 is now available from:

  <https://download.litercoin.org/litercoin-0.18.1/>
...
`/Applications/Litercoin-Qt` (on Mac) or `litercoind`/`litercoin-qt` (on
Linux).
...
Litercoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Litercoin Core on unsupported systems.
...
10.10. Additionally, Litercoin Core does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/minblock/litercoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:8803:8803` (this is an extra `:8803` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@litercoin.org) the Litercoin Core
...
- A new short [document](https://github.com/minblock/litercoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/minblock/litercoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/minblock/litercoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/minblock/litercoin/blob/master/doc/litercoin-conf.md)
  about the `litercoin.conf` file describes how to use it to configure
  Litercoin Core.

- A new document introduces Litercoin Core's BIP174 [Partially-Signed
  Litercoin Transactions
  (PSBT)](https://github.com/minblock/litercoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/minblock/litercoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Litercoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Litercoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/minblock/litercoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/minblock/litercoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Litercoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Litercoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/minblock/litercoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/minblock/litercoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart litercoind with the `-deprecatedrpc=generate`
...
- A new `litercoin-wallet` tool is now distributed alongside Litercoin
...
This section describes planned changes to Litercoin Core that may affect
other Litercoin software and services.

- Since version 0.16.0, Litercoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Litercoin Core 0.20 (expected about
  a year after 0.18), Litercoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Litercoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Litercoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Litercoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Litercoin Core
...
- A sub-project of Litercoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Litercoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Litercoin Core's own implementation, although entropy
  gathered by Litercoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Litercoin Core a
...
- On macOS, Litercoin Core now opts out of application CPU throttling
...
- #8803 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
