...
`/Applications/Litercoin-Qt` (on Mac) or `litecoind`/`litecoin-qt` (on
Linux).
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@litecoin.org) the Litercoin Core
...
- A new `litecoin-wallet` tool is now distributed alongside Litercoin
